...
func TestSlicePointerBinding(t *testing.T) {
	t.Run("without OmitSliceElementPointers", func(t *testing.T) {
		...
		require.Equal(t, ta.GO.String(), "[]*github.com/swiftcarrot/dashi/gqlgen/example/chat.Message")
	})
...
func createBinder(cfg Config) (*Binder, *ast.Schema) {
	cfg.Models = TypeMap{
		"Message": TypeMapEntry{
			Model: []string{"github.com/swiftcarrot/dashi/gqlgen/example/chat.Message"},
		},
	}
